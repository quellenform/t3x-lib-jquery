...
/**
 * Add jQuery on top of the HTML-output
 *
...
class PageRenderer
{
    ...
    public function renderPreProcess($params, $pObj)
    {
        ...
        if (!empty($conf['cdn']) && array_key_exists($conf['cdn'], $this->jQueryCdnUrls)) {
            ...
            if ($conf['cdn'] === 'local') {
                ...
                $file = sprintf(
                        $this->jQueryCdnUrls['local'], VersionNumberUtility::convertIntegerToVersionNumber($versionLocal), $minPart
                    ) . $gzipPart;
                $file = str_replace(PATH_site, '', GeneralUtility::getFileAbsFileName($file));
            } else {
                // Get CDN and replace placeholders
                $file = sprintf(
                    $this->jQueryCdnUrls[$conf['cdn']], VersionNumberUtility::convertIntegerToVersionNumber($versionCdn), $minPart
                );
     ...
     *
     * @param integer $version string representation of the selected version number
     * @return integer
     */
